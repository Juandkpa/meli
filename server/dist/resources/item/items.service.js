...
const getCategories = async (data) => {
  ...
  const category = data.filters.find(({ id }) => id === "category"); //Documentar esto
...
const getItemsByQuery = async (query) => {
  ...
  const { data } = await _axios.default.get(
    encodeURI(`${apiEnpoint}/sites/MLA/search?q=${query}`)
  );
  return {
    author,
...
const getItemDetail = async (id) => {
  const requests = [
    _axios.default.get(encodeURI(`${apiEnpoint}/items/${id}`)),
    _axios.default.get(encodeURI(`${apiEnpoint}/items/${id}/description`)),
  ];
